...
import { formatDateRange, formatStartDate } from '@/lib/formatDate';
...
export default function PerformanceCard({ event, onClick }: PerformanceCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 md:p-6 space-y-2 flex-1 flex flex-col">
        {/* Title */}
        <h3 className="text-lg md:text-xl font-semibold leading-snug group-hover:underline" style={{ fontFamily: 'var(--font-family-display)' }}>
          {event.title}
        </h3>
      ...
      <div className="flex items-center justify-between p-3 md:p-4 bg-white">
        <p className="text-[10px] md:text-xs text-gray-700 font-medium whitespace-nowrap">{formatStartDate(event.startDate)}</p>
